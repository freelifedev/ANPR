...
class LicensePlateDetector:
    ...
    def detectCharacterCandidates(self, region):
        ...
        cv2.imshow("Original Candiates", charCandidates)

        # if there are more charter candidates then the supplied number, the prune
        # the candidates
        if len(cnts) > self.numChars:
            (charCandidates, cnts) = self.pruneCandiates(charCandidates, cnts)
    ...
    def pruneCandiates(self, charCandiates, cnts):
        # initialize the pruned candidates mask and the list of dimensions
        prunedCandidates = np.zeros(charCandiates.shape, dtype="uint8")
        dims = []
        ...
        for i in xrange(0, len(dims)):
            ...
            print "dims[{}] = {} , dims - dims[{}] = {}".format(i, dims[i], i , dims - dims[i])

        print "diffs = {}".format(diffs)


        # find te top number of candidates with the most similar dimensions and loop over
        ...
        for i in np.argsort(diffs)[:self.numChars]:
            ...
            print "index i ({}) should be zigzag".format(i)
    ...
    def scissor(self, lp):
        ...
        # loop over the contours
        for c in cnts:
            # compute the boundng box for the contour while maintaining the minimum width
            (boxX, boxY, boxW, boxH) = cv2.boundingRect(c)
        ...
        # loop over th started bounding boxes
        for (startX, startY, endX, endY) in boxes:
            # extract the ROI from the the thresholded license plate and update the characters
            # list
