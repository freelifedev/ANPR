...
ap.add_argument("-d", "--digit-classifier", required=True, help="path to the output digit clasifier")
...
for imagePath in sorted(list(paths.list_images(args["images"]))):
    ...
    # initialize the license plate detector and dtect the license plates and  characters
    lpd = LicensePlateDetector(image, numChars=7)
    plates = lpd.detect()

    # loop over the detected plates
    for (lpBox, chars) in plates:
        # initialize the txt containing the recongnized characters
        text = ""
        ...
        for (i , char) in enumerate(chars):
            ...
            # update the text of recognized characers
            text += prediction.upper()

        # only draw the characters and bounding box if there are some characters
        # we can dispaly
        if len(chars) > 0 :
